...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10" x-data="{ openApprove: false, openRevise: false }">
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="<?php echo e(route('monevakademik.tashih.index')); ?>" class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition border border-gray-300 shadow-sm dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div>
                    ...
                    <strong class="text-base text-gray-900 dark:text-white"><?php echo e($proposal->course->course_name); ?></strong>
                    <div class="text-xs text-gray-500 mt-0.5"><?php echo e($proposal->course_id); ?></div>
                </div>
                <div>
                    <span class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">Dosen Pengaju</span>
                    <strong class="text-base text-gray-900 dark:text-white"><?php echo e($proposal->creator->name ?? '-'); ?></strong>
                </div>
                <div>
                    <span class="block text-[11px] font-bold text-gray-500 uppercase tracking-widest mb-1">Jenis Ujian</span>
                    <strong class="text-base text-gray-900 dark:text-white"><?php echo e($proposal->exam_type); ?></strong>
                </div>
                <div>
                    ...
                    ?>
                    <span class="inline-flex rounded-md px-3 py-1.5 text-xs font-bold border <?php echo e($statusColor); ?>">
                        <i class="fa-solid <?php echo e($proposal->status == 'APPROVED' ? 'fa-check' : ($proposal->status == 'REVISED' ? 'fa-pen-to-square' : 'fa-clock')); ?> mr-1.5"></i> 
                        <?php echo e($proposal->status); ?>
        ...
        <?php if($proposal->reviews && $proposal->reviews->count() > 0): ?>
        <div class="rounded-xl border border-orange-200 bg-orange-50 p-5 shadow-sm dark:bg-orange-900/20 dark:border-orange-500/30">
            <h4 class="text-sm font-bold text-orange-800 dark:text-orange-400 flex items-center gap-2 mb-2">
                <i class="fa-solid fa-clock-rotate-left"></i> Catatan Revisi Terakhir (<?php echo e($proposal->reviews->first()->created_at->format('d M Y')); ?>)
            </h4>
            <p class="text-sm text-orange-700 dark:text-orange-300 italic">"<?php echo e($proposal->reviews->first()->comment); ?>"</p>
        </div>
        <?php endif; ?>
            ...
            <div class="space-y-4">
                ...
                <div class="rounded-xl border-l-4 border-l-indigo-500 border-y border-r border-gray-200 bg-white p-6 shadow-sm transition hover:shadow-md dark:border-y-gray-700 dark:border-r-gray-700 dark:bg-gray-800">
                    <div class="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4 border-b border-gray-100 pb-4 dark:border-gray-700">
                        <strong class="text-gray-900 dark:text-white text-lg">Soal No. <?php echo e($eq->order_no); ?></strong>
                        <div class="flex flex-wrap gap-2">
                            <span class="inline-flex items-center rounded-md bg-indigo-50 px-2.5 py-1 text-xs font-bold text-indigo-700 border border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-400">
                                <i class="fa-solid fa-bullseye mr-1.5 opacity-70"></i> CPMK: <?php echo e($eq->question->cpmk_id); ?>
                            ...
                            <span class="inline-flex items-center rounded-md bg-gray-100 px-2.5 py-1 text-xs font-bold text-gray-700 border border-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600">
                                <i class="fa-solid fa-weight-hanging mr-1.5 opacity-70"></i> Bobot: <?php echo e($eq->weight); ?>%
                            </span>
                        </div>
                    </div>
                    <p class="text-gray-700 dark:text-gray-300 whitespace-pre-line leading-relaxed"><?php echo e($eq->question->question_text); ?></p>
                </div>
        ...
        <?php if(Auth::id() == $proposal->created_by && in_array($proposal->status, ['SUBMITTED', 'REVISED'])): ?>
        <div class="mt-8 rounded-xl border border-gray-200 bg-gray-50 p-6 shadow-sm dark:bg-gray-800/50 dark:border-gray-700" x-data="{ openDelete: false }">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div class="flex gap-3">
                    <a href="<?php echo e(route('monevakademik.tashih.edit', $proposal->uuid)); ?>" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white shadow-md hover:bg-blue-700 transition">
            ...
            <div x-show="openDelete" class="fixed inset-0 z-[999999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm"
                ...
                <div @click.away="openDelete = false" class="w-full max-w-md transform rounded-2xl bg-white p-6 shadow-2xl text-center dark:bg-gray-800">
                    ...
                    <form action="<?php echo e(route('monevakademik.tashih.destroy', $proposal->uuid)); ?>" method="POST" class="flex justify-center gap-3">
                        <?php echo csrf_field(); ?>
        ...
        <?php if(Auth::id() != $proposal->created_by && $proposal->status == 'SUBMITTED'): ?>
        ...
        <div x-show="openApprove" class="fixed inset-0 z-[999999] flex items-center justify-center overflow-y-auto bg-black/50 p-4 backdrop-blur-sm"
            ...
            <div @click.away="openApprove = false" class="relative w-full max-w-lg transform rounded-2xl bg-white p-8 shadow-2xl ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700 transition-all">
                ...
                <form action="<?php echo e(route('monevakademik.tashih.approve', $proposal->uuid)); ?>" method="POST" id="approveForm">
                    <?php echo csrf_field(); ?>
        ...
        <div x-show="openRevise" class="fixed inset-0 z-[999999] flex items-center justify-center overflow-y-auto bg-black/50 p-4 backdrop-blur-sm"
            ...
            <div @click.away="openRevise = false" class="relative w-full max-w-lg transform rounded-2xl bg-white p-8 shadow-2xl ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700 transition-all">
                ...
                <form action="<?php echo e(route('monevakademik.tashih.revise', $proposal->uuid)); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    <div class="mb-4">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\sainteku\Modules/MonevAkademik\resources/views/tashih/show.blade.php ENDPATH**/ ?>