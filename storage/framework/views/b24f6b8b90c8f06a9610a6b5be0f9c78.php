<template x-teleport="#modal-root">
    <div x-data="openCourseCreateModal()" @open-create-modal.window="openCreate = true" x-show="openCreate"
        ...
        x-init="if (Object.keys(<?php echo \Illuminate\Support\Js::from($errors->messages())->toHtml() ?>).length > 0) openCreate = true">
        ...
        <div @click.away="openCreate = false"
            ...
            <form x-show="createMode === 'manual'" action="<?php echo e(route('masterdata.courses.store')); ?>" method="POST" class="flex flex-col flex-1 overflow-hidden">
                <?php echo csrf_field(); ?>
                        ...
                        <div class="p-5 grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="course_name" required maxlength="100" placeholder="Contoh: Pemrograman Web"
                                    ...
                                    class="w-full rounded-xl border-gray-300 bg-gray-50 px-4 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:bg-[#0f172a] dark:text-white dark:border-gray-600 <?php $__errorArgs = ['course_name'];
                            ...
                            <div>
                                ...
                                <select x-model="createFakultas" @change="loadCreateProdis()" required
                                    ...
                                    <?php $__currentLoopData = $faculties; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fak): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($fak->id); ?>" <?php if(old('fakultas_id') == $fak->id): echo 'selected'; endif; ?>><?php echo e($fak->unit_name); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="unit_id" x-model="createProdi" required :disabled="!createFakultas"
                                    class="w-full rounded-xl border-gray-300 bg-gray-50 px-4 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 disabled:opacity-60 dark:bg-[#0f172a] dark:text-white dark:border-gray-600 <?php $__errorArgs = ['unit_id'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?> border-red-500 <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="is_active" required
                                    class="w-full rounded-xl border-gray-300 bg-gray-50 px-4 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:bg-[#0f172a] dark:text-white dark:border-gray-600">
                                    <option value="1" <?php if(old('is_active') == '1' || !$errors->any()): echo 'selected'; endif; ?>>Aktif</option>
...
<script>
    function openCourseCreateModal() {
        return {
            openCreate: false,
            createMode: 'manual',
            createFakultas: <?php echo \Illuminate\Support\Js::from(old('fakultas_id', ''))->toHtml() ?>,
            createProdi: <?php echo \Illuminate\Support\Js::from(old('unit_id', ''))->toHtml() ?>,
            createProdis: [],
            bulkFakultas: '',
            bulkProdi: '',
            bulkProdis: [],
            ...
            bulkImporting: false,
            bulkResult: null,
            bulkStoreUrl: <?php echo json_encode(route('masterdata.courses.bulk.store'), 15, 512) ?>,
            templateUrl: <?php echo json_encode(route('masterdata.courses.bulk.template'), 15, 512) ?>,
            ...
            async loadCreateProdis(keepProdi = false) {
                ...
                try {
                    const response = await fetch(`<?php echo e(route('masterdata.courses.api.prodis')); ?>?fakultas_id=${this.createFakultas}`, {
            ...
            async loadBulkProdis() {
                ...
                try {
                    const response = await fetch(`<?php echo e(route('masterdata.courses.api.prodis')); ?>?fakultas_id=${this.bulkFakultas}`, {
                        headers: { 'Accept': 'application/json' },
                    });
                    if (response.ok) {
...
</script><?php /**PATH /mnt/volume_sgp1_1781186006004/projects/sainteku/Modules/MasterData/resources/views/courses/modal-create.blade.php ENDPATH**/ ?>