            ...
            <div class="shrink-0 flex items-center justify-between border-b border-gray-200 bg-white px-4 sm:px-8 py-3 sm:py-4 z-20 dark:bg-[#1e293b] dark:border-gray-700 shadow-sm sticky top-0">
                <div class="flex min-w-0 flex-1 items-center gap-2 sm:gap-3 pr-4">
                    <div class="flex h-8 w-8 sm:h-9 sm:w-9 shrink-0 items-center justify-center rounded-xl border border-blue-100 bg-blue-50 dark:border-blue-800/50 dark:bg-blue-900/30">
                        <i class="fa-solid fa-calendar-plus text-sm text-blue-600 dark:text-blue-400 sm:text-base"></i>
                    </div>
                    <div class="min-w-0 leading-tight">
                        <h3 class="truncate text-sm font-bold text-gray-900 dark:text-gray-100 sm:text-xl">Tambah Tahun Akademik</h3>
                        <p class="truncate text-xs text-gray-500 dark:text-gray-400 sm:text-sm">Daftarkan periode akademik baru untuk modul ujian dan monev</p>
                    </div>
                </div>
                <div class="shrink-0 w-8 sm:w-9"></div>
            </div>

            <form action="{{ route('masterdata.periods.store') }}" method="POST" class="flex flex-col flex-1 overflow-hidden">
                @csrf
                    ...
                    <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-[#1e293b]">
                        ...
                        <div class="p-5 grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <input type="text" name="name" required maxlength="15" pattern="\d{4}/\d{4}"
                                    ...
                                    placeholder="Contoh: 2025/2026" value="{{ old('name') }}">
                                <p class="mt-1.5 text-xs text-gray-500">Format: YYYY/YYYY</p>
                            </div>
                            <div>
                                <label class="mb-2 block text-[10px] font-bold uppercase tracking-widest text-gray-400">
                            ...
                            <div class="md:col-span-2">
                                <label class="relative inline-flex cursor-pointer items-center gap-3">
                                    ...
                                    <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">Jadikan periode aktif</span>
                                </label>
                                <p class="mt-1.5 text-xs text-gray-500">Hanya satu periode yang boleh aktif. Mengaktifkan ini akan menonaktifkan periode lain.</p>
                            </div>
                        </div>
                    </div>
                </div>
