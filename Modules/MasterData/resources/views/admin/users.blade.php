...
@section('content')
    <div class="space-y-6" x-data="usersApp()" x-init="initData()"
        @users-bulk-imported.window="handleBulkImported($event)" x-cloak>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div class="flex flex-nowrap items-center gap-3">
                <div class="relative min-w-0 flex-1">
                    ...
                    <input type="text" x-model="searchQuery" @input.debounce.400ms="fetchUsers()"
                ...
                <div class="flex shrink-0 items-center gap-2">
                    ...
                    <select x-model="perPageFilter" @change="fetchUsers(1)" title="Jumlah data per halaman"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr x-show="usersList.length === 0 && !isLoading">
                            ...
                            <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1">
                                        ...
                                        <span x-show="user.roles.length === 0" class="text-xs italic text-gray-400">No Role</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 w-[20%] max-w-[150px]">
                                    <span x-show="user.unit_id"
                                        :title="user.unit_name || 'Unit tidak ditemukan'"
                                        class="inline-block max-w-full truncate rounded-md bg-indigo-50 px-2.5 py-1 align-middle text-xs font-semibold text-indigo-700 ring-1 ring-inset ring-indigo-700/10 cursor-help dark:bg-indigo-900/30 dark:text-indigo-400"
                                        x-text="user.unit_id"></span>
                                    <span x-show="!user.unit_id" class="text-xs font-medium italic text-gray-400">-</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span x-show="user.is_active == '1'"
        ...
        {{-- Floating Filter FAB (teleport ke body biar fixed pojok kanan bawah viewport) --}}
        ...
        <div class="fixed z-[9990] flex flex-col items-end gap-3"
            ...
            <div x-show="filterFabOpen" x-cloak
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select x-model="sortFilter" @change="fetchUsers(1)"
                    ...
                    <div>
                        <label class="mb-1.5 block text-[10px] font-bold uppercase tracking-widest text-gray-400">Status Akun</label>
                        <select x-model="statusFilter" @change="fetchUsers()"
                            class="w-full rounded-xl border-gray-300 bg-gray-50 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:bg-[#0f172a] dark:border-gray-600 dark:text-white">
                    ...
                    <div>
                        <label class="mb-1.5 block text-[10px] font-bold uppercase tracking-widest text-gray-400">Role</label>
                    ...
                    <div>
                        <label class="mb-1.5 block text-[10px] font-bold uppercase tracking-widest text-gray-400">Unit</label>
                        <select x-model="unitFilter" @change="fetchUsers()"
                            class="w-full rounded-xl border-gray-300 bg-gray-50 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:bg-[#0f172a] dark:border-gray-600 dark:text-white">
                            <option value="">Semua unit</option>
                            <template x-for="unit in unitsList" :key="unit.id">
                                <option :value="unit.id" x-text="unit.id + ' — ' + unit.name"></option>
                            </template>
                        </select>
                    </div>
                </div>
            ...
            <button type="button" @click="filterFabOpen = !filterFabOpen"
                class="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-indigo-600 text-white shadow-lg shadow-indigo-600/30 outline-none ring-0 ring-offset-0 transition hover:bg-indigo-700 hover:shadow-xl focus:outline-none focus:ring-0 focus:ring-offset-0 active:outline-none active:ring-0"
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('usersApp', () => ({
                ...
                unitFilter: '',
                filterFabOpen: false,
                rolesList: @json($roles->map(fn ($r) => ['id' => $r->id, 'name' => $r->role_name])->values()),
                unitsList: @json($units->map(fn ($u) => ['id' => $u->id, 'name' => $u->unit_name])->values()),
                usersList: [],
                isLoading: false,
                pagination: {},
                alert: { type: '', message: '' },
                ...
                async fetchUsers(page = 1) {
                    ...
                    try {
                        const response = await fetch(`{{ route('masterdata.admin.users.api.data') }}?${params.toString()}`, {
                            headers: {
                    ...
                    window.dispatchEvent(new CustomEvent('open-detail-modal', {
                        ...
                        detail: {
                            ...
                            userData: {
                                ...
                                type: user.user_type,
                                unit: user.unit_id,
                                tingkatUtama: user.tingkat_utama,
                                active: user.is_active == '1',
                                roles: user.role_ids.map(String),
                                unitTambahan: user.unit_tambahan.map(String),
                            },
                        },
                    }));
                },
