...
@section('content')
    <div class="space-y-6" x-data="rolesApp()" x-init="initData()" x-cloak>
        ...
        <div class="flex flex-col gap-4 pb-4 border-b border-gray-200 sm:flex-row sm:items-center sm:justify-between dark:border-gray-700">
            <div>
                <h2 class="flex items-center gap-2 text-2xl font-bold text-gray-900 dark:text-white">
                    <i class="fa-solid fa-shield-halved text-indigo-500"></i> Manajemen Role
                </h2>
                <nav>
                    <ol class="flex items-center gap-2 mt-1 text-sm font-medium text-gray-500 dark:text-gray-400">
                        <li>Master Data /</li>
                        <li class="text-indigo-600 dark:text-indigo-400">Role</li>
                    </ol>
                </nav>
            </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div class="flex flex-nowrap items-center gap-3">
                <div class="relative min-w-0 flex-1">
                    ...
                    <input type="text" x-model="searchQuery" @input.debounce.400ms="fetchRoles()"
                ...
                <div class="flex shrink-0 items-center gap-2">
                    ...
                    <select x-model="perPageFilter" @change="fetchRoles(1)" title="Jumlah data per halaman"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400">
                    <thead class="text-xs uppercase bg-gray-50 text-gray-700 dark:bg-gray-700/50 dark:text-gray-300">
                        <tr>
                            <th class="px-6 py-4 font-semibold">Info Role</th>
                            <th class="px-6 py-4 font-semibold">Hak Akses</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr x-show="rolesList.length === 0 && !isLoading">
                            <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                <i class="mb-3 text-3xl opacity-50 fa-solid fa-shield-halved"></i><br>
                        ...
                        <template x-for="role in rolesList" :key="role.id">
                            <tr class="transition hover:bg-gray-50 dark:hover:bg-gray-700/30">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="flex items-center justify-center flex-shrink-0 w-10 h-10 font-bold text-indigo-600 bg-indigo-100 rounded-full dark:bg-indigo-900/40 dark:text-indigo-400"
                                            x-text="role.initial"></div>
                                        <div>
                                            <div class="font-medium text-gray-900 dark:text-white" x-text="role.role_name"></div>
                                            <div class="text-xs font-mono text-gray-500 dark:text-gray-400" x-text="role.role_code"></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex rounded-full bg-purple-50 px-2.5 py-0.5 text-xs font-medium text-purple-700 border border-purple-200 dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800">
                                        <span x-text="role.permission_count"></span> izin
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300"
                                        x-text="role.user_count + ' user'"></span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span x-show="role.is_active == '1'"
                                ...
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    <button type="button" @click="openDetail(role)"
        ...
        @include('masterdata::roles.modal-detail')
            ...
            <div x-show="filterFabOpen" x-cloak
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select x-model="sortFilter" @change="fetchRoles(1)"
                    ...
                    <div>
                        ...
                        <select x-model="statusFilter" @change="fetchRoles(1)"
                            class="w-full rounded-xl border-gray-300 bg-gray-50 px-3 py-2 text-sm outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 dark:bg-[#0f172a] dark:border-gray-600 dark:text-white">
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('rolesApp', () => ({
                ...
                async fetchRoles(page = 1) {
                    ...
                    try {
                        const response = await fetch(`{{ route('masterdata.roles.api.data') }}?${params.toString()}`, {
                            headers: {
                ...
                openDetail(role) {
                    window.dispatchEvent(new CustomEvent('open-detail-modal', {
                        ...
                        detail: {
                            url: role.update_url,
                            deleteUrl: role.delete_url,
                            permissionsUrl: role.permissions_url,
                            roleName: role.role_name,
                            canDelete: role.can_delete,
                            assigned: role.assigned_permissions,
                            roleData: {
                                id: role.id,
                                code: role.role_code,
                                name: role.role_name,
                                active: role.is_active == '1',
