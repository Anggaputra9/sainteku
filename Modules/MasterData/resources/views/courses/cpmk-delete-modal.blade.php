<template x-teleport="#modal-root">
    <div x-data="cpmkDeleteModal()" @open-cpmk-delete-modal.window="handleOpen($event)" x-show="open"
        @click.self="close()"
            ...
            <div class="border-b border-gray-200 bg-white px-4 py-3 sm:px-6 sm:py-4 dark:bg-[#1e293b] dark:border-gray-700">
                <div class="flex min-w-0 items-center gap-2 sm:gap-3">
                    ...
                    <div class="min-w-0 leading-tight">
                        <h3 class="truncate text-sm font-bold text-gray-900 dark:text-gray-100 sm:text-lg">Konfirmasi Hapus CPMK</h3>
                    ...
                    <template x-for="(item, index) in items" :key="item.id">
                        <div class="flex items-start gap-3 border-b border-gray-100 px-3 py-2.5 last:border-b-0 dark:border-gray-700/60">
                            ...
                            <div class="min-w-0 flex-1">
                                <span class="font-bold font-mono text-xs text-indigo-700 dark:text-indigo-300" x-text="item.id"></span>
    ...
    function cpmkDeleteModal() {
        return {
            ...
            handleOpen(event) {
                ...
                this.courseId = String(detail.courseId || detail.items?.[0]?.course_id || '').trim();
                this.bulkUrl = detail.bulkUrl || (this.courseId ? `/masterdata/courses/${this.courseId}/cpmk/bulk-delete` : '');
                this.deleting = false;

                if (this.mode === 'bulk') {
                    this.subtitle = `${this.items.length} CPMK terpilih`;
                    this.message = `Apakah Anda yakin ingin menghapus ${this.items.length} CPMK terpilih? Tindakan ini tidak dapat dibatalkan.`;
                    return;
                }

                const item = this.items[0] || {};
                this.subtitle = item.id || '';
                this.message = `Apakah Anda yakin ingin menghapus CPMK ${item.id || ''}? Tindakan ini tidak dapat dibatalkan.`;
            },
            ...
            resolveBulkUrl() {
                ...
                if (this.courseId) {
                    return `/masterdata/courses/${this.courseId}/cpmk/bulk-delete`;
                }

                const courseId = String(this.items?.[0]?.course_id || '').trim();
                return courseId ? `/masterdata/courses/${courseId}/cpmk/bulk-delete` : '';
            },
