                                ...
                                @forelse($documents as $doc)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition items-start">
                                        <td class="px-6 py-4 align-top">
                                            ...
                                            {{-- Badge Sifat Dokumen & PPID --}}
                                            <div class="flex flex-wrap gap-2 mt-2">
                                                @if($doc->sifat_dokumen === 'Publik')
                                        ...
                                        <td class="px-6 py-4 text-center align-top">
                                            <div class="flex flex-col gap-2 items-center">
                                                ...
                                                @if ($doc->status == 4)
                                                    {{-- Status Ditolak/Revisi -> Tombol Revisi File (hanya untuk dokumen milik sendiri) --}}
                                                    @if(Auth::user()->hasPermission(1, 'U') && $doc->created_by === auth()->id())
                                                    <button
                                                ...
                                                @elseif ($doc->status == 3)
                                                    ...
                                                    @endif
                                                @else
                                                    {{-- Status Draft / Menunggu Review -> Terkunci --}}
                                                    <button disabled
                                                ...
                                                {{-- Tombol Edit & Hapus (hanya untuk dokumen milik sendiri) --}}
                                                <div class="flex gap-2">
                                                    {{-- Tombol Edit --}}
                                                    @if(Auth::user()->hasPermission(1, 'U') && $doc->created_by === auth()->id())
                                                    <button
                                                        @click="openEdit = true; editUrl = '{{ route('DocumentRepository.update', $doc->id) }}'; editData = {{ json_encode([
                                                    ...
                                                    {{-- Tombol Hapus (hanya untuk dokumen milik sendiri yang belum disetujui) --}}
                                                    @if(Auth::user()->hasPermission(1, 'D') && $doc->status != 3 && $doc->created_by === auth()->id())
                                                    <button
                                                        @click="openDelete = true; deleteUrl = '{{ route('DocumentRepository.destroy', $doc->id) }}'; deleteTitle = '{{ addslashes($doc->document_title) }}'"
