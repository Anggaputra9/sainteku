...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10" x-data="{ 
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center gap-2 mb-2">
            <a href="{{ route('manajementinfrastruktur.persetujuan.index', ['status' => 'all']) }}" 
            ...
            <a href="{{ route('manajementinfrastruktur.persetujuan.index', ['status' => 'pending']) }}" 
            ...
            <a href="{{ route('manajementinfrastruktur.persetujuan.index', ['status' => 'approved']) }}" 
            ...
            <a href="{{ route('manajementinfrastruktur.persetujuan.index', ['status' => 'returned']) }}" 
                class="px-4 py-2 text-sm font-semibold rounded-full transition {{ $filterStatus == 'returned' ? 'bg-blue-500 text-white shadow-md' : 'bg-white text-blue-600 border border-blue-200 hover:bg-blue-50 dark:bg-gray-800 dark:border-blue-900/50 dark:hover:bg-blue-900/30' }}">
                <i class="fa-solid fa-rotate-left mr-1"></i> Telah Dikembalikan
            </a>
            <a href="{{ route('manajementinfrastruktur.persetujuan.index', ['status' => 'rejected']) }}" 
                        ...
                        @forelse($peminjaman as $item)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition items-start">
                                <td class="px-6 py-4 align-top">
                                    <div class="font-bold text-gray-900 dark:text-white text-base">{{ $item->user_name }}</div>
                                    <div class="text-sm font-medium text-blue-600 dark:text-blue-400 mt-0.5">{{ $item->loan_code }}</div>
                                    <div class="text-xs text-gray-500 mt-1.5 opacity-80">Tujuan: {{ $item->purpose }}</div>
                                    
                                    @if ($item->status == 2 && $item->admin_note)
                                        <div class="mt-2 text-[11px] text-red-600 font-medium">Alasan Tolak: {{ $item->admin_note }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 align-top">
                                    <div class="font-semibold text-indigo-600 dark:text-indigo-400">{{ $item->item_name }}</div>
                                    <div class="text-xs text-gray-500 mt-0.5 mb-2">Jumlah: {{ $item->quantity }} Item</div>
                                    
                                    <div class="flex flex-col gap-1 text-[11px] font-medium text-gray-700 dark:text-gray-300">
                                        <div><i class="fa-solid fa-plane-departure text-gray-400 mr-1 w-3"></i> {{ \Carbon\Carbon::parse($item->start_date)->format('d M Y, H:i') }}</div>
                                        <div><i class="fa-solid fa-plane-arrival text-gray-400 mr-1 w-3"></i> {{ \Carbon\Carbon::parse($item->end_date)->format('d M Y, H:i') }}</div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center align-top">
                                    <div class="flex justify-center gap-2">
                                        @if($item->status == 0) 
                                            {{-- Tombol Review untuk status Pending --}}
                                            <button 
                                                @click="openReview = true; 
                                                        reviewUrl = '{{ route('manajementinfrastruktur.persetujuan.update', $item->id) }}'; 
                                                        loanCode = '{{ $item->loan_code }}'; 
                                                        userName = '{{ addslashes($item->user_name) }}';
                                                        itemName = '{{ addslashes($item->item_name) }}';
                                                        purpose = '{{ addslashes($item->purpose) }}'"
                                        ...
                                        @elseif($item->status == 1)
                                            {{-- Tombol Tandai Dikembalikan untuk status Approved --}}
                                            <form action="{{ route('manajementinfrastruktur.persetujuan.update', $item->id) }}" method="POST" onsubmit="return confirm('Tandai barang ini sudah dikembalikan dan pulihkan stok?');">
                                                @csrf
                                                @method('PUT')
