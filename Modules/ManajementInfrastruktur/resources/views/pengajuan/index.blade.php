        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div x-data="{ openCreate: false }">
                    ...
                    {{-- Memanggil komponen file modal create --}}
                    @include('manajementinfrastruktur::pengajuan.modal-create')
                </div>
            </div>
                ...
                <div class="flex items-center w-full border-l-4 border-red-500 bg-red-50 p-4 shadow-sm dark:bg-gray-800 dark:border-red-400 rounded-r-lg">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-red-700 dark:text-red-500">
                            {{ session('error') ?? 'Gagal memproses pengajuan. Periksa kembali form isian Anda.' }}
                        </p>
                    </div>
                </div>
            @endif
            ...
            <div class="flex flex-wrap items-center gap-2 mb-2">
                ...
                <a href="{{ route('manajementinfrastruktur.pengajuan.index', ['status' => 'pending']) }}"
                ...
                <a href="{{ route('manajementinfrastruktur.pengajuan.index', ['status' => 'approved']) }}"
                ...
                <a href="{{ route('manajementinfrastruktur.pengajuan.index', ['status' => 'rejected']) }}"
                ...
                <a href="{{ route('manajementinfrastruktur.pengajuan.index', ['status' => 'returned']) }}"
                            ...
                            @forelse($riwayat as $item)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition items-start">
                                    <td class="px-6 py-4 align-top">
                                        <div class="font-bold text-gray-900 dark:text-white text-base">{{ $item->loan_code }}</div>
                                        <div class="text-sm font-medium text-blue-600 dark:text-blue-400 mt-0.5">{{ $item->item_name }}</div>
                                        <div class="text-xs text-gray-500 mt-1.5"><i class="fa-solid fa-boxes-stacked mr-1"></i> Jumlah: {{ $item->quantity }} Item</div>

                                        {{-- Kotak Catatan Penolakan (Mirip Catatan Revisi) --}}
                                            ...
                                            <div class="mt-4 w-full rounded-lg border border-red-200 bg-red-50/80 p-3.5 dark:border-red-500/30 dark:bg-red-500/10 shadow-sm">
                                                <div class="flex items-start gap-3">
                                                    ...
                                                    <div class="flex-col">
                                                        ...
                                                        <p class="text-sm font-medium text-red-800 dark:text-red-200 leading-relaxed">{{ $item->admin_note }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                    ...
                                    <td class="px-6 py-4 text-center align-top">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center gap-1.5 rounded-md px-3 py-1.5 text-xs font-bold border {{ $statusColor }}">
