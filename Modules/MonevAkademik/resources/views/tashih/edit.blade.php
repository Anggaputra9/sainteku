...
@section('content')
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10"
        x-data="{ openBankSoal: false, bankSoalList: [], searchQuery: '', isLoading: false, fetchBankSoal() { this.isLoading = true; fetch('{{ route('monevakademik.tashih.api.banksoal', $course->id) }}').then(res => res.json()).then(data => { this.bankSoalList = data; this.isLoading = false; }); }, get filteredBankSoal() { if (this.searchQuery === '') return this.bankSoalList; return this.bankSoalList.filter(q => q.question_text.toLowerCase().includes(this.searchQuery.toLowerCase())); }, useQuestion(q) { addQuestionCard({ text: q.question_text, cpmk: q.cpmk_id, weight: '' }); this.openBankSoal = false; this.searchQuery = ''; } }">
        ...
        <div class="space-y-6">
            ...
            <form id="form-pengajuan" action="{{ route('monevakademik.tashih.update', $proposal->uuid) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="exam_type"
                                ...
                                <option value="UTS" {{ $proposal->exam_type == 'UTS' ? 'selected' : '' }}>Ujian Tengah
                                    Semester (UTS)</option>
                ...
                <div
                    ...
                    <div class="flex gap-3">
                        <a href="{{ route('monevakademik.tashih.show', $proposal->uuid) }}"
    ...
    <script>
        ...
        const existingQuestions = @json($proposal->examQuestions->map(function ($eq) {
            return [
                'text' => $eq->question->question_text,
                'cpmk' => $eq->question->cpmk_id,
                'weight' => $eq->weight
            ];
        }));
