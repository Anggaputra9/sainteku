...
<div x-show="openDetail"
    ...
    <div
        ...
        {{-- Header dengan Tombol Kembali Kuning di Atas --}}
        <div
        ...
        {{-- KUNCI UTAMA: BUNGKUS PAKAI FORM BIAR SEKALI SUBMIT --}}
        <form method="POST" id="bulkReviewForm" class="flex-1 flex flex-col overflow-hidden relative">
            ...
            <div class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 custom-scrollbar bg-slate-50 dark:bg-[#0f172a]"
                ...
                <template x-if="selectedProposal">
                    <div class="space-y-6">
                        ...
                        {{-- Alert Revisi Umum (Jika ada histori) --}}
                        <template
                        ...
                        <div class="space-y-6 mt-4">
                            <template x-for="(eq, index) in selectedProposal.exam_questions" :key="index">
                                <div
                                    ...
                                    {{-- FORM INPUT KOMENTAR PER-SOAL (Hanya Reviewer) --}}
                        ...
                        <template x-if="isReviewer && selectedProposal.status === 'SUBMITTED'">
                            <div
                                ...
                                <div class="p-6 sm:p-8">
                                    {{-- Form Catatan Dibuat Full Width --}}
                                    <div>
                                        <label
            ...
            {{-- Footer Modal: Tempat Semua Tombol Aksi Berada --}}
            <div
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    {{-- Area Kanan (Aksi Dinamis Berdasarkan Role/Status) --}}
                    <div class="w-full sm:w-auto flex flex-col sm:flex-row gap-3">

                        {{-- Aksi Dosen (Creator) -- FIX: Cek null dengan selectedProposal && --}}
                        <template
                        ...
                        {{-- Aksi Reviewer (Kaprodi) -- FIX: Cek null dengan selectedProposal && --}}
                        <template x-if="isReviewer && selectedProposal && selectedProposal.status === 'SUBMITTED'">
                            <div class="flex flex-col sm:flex-row gap-3 w-full">
                                ...
                                {{-- Logic Pemanggilan Modal TTD vs Langsung Submit --}}
                                @if(empty(Auth::user()->signature))
...
{{-- MODAL KONFIRMASI HAPUS / BATALKAN PENGAJUAN --}}
<div x-show="openDelete"
    ...
    <div @click.away="openDelete = false" x-transition:enter="transition ease-out duration-300"
        ...
        <form method="POST" id="formDeleteProposal"
            :action="'{{ url('monev-akademik/tashih/destroy') }}/' + (selectedProposal ? selectedProposal.uuid : '')">
            @csrf
            @method('DELETE')
...
@if(empty(Auth::user()->signature))
    {{-- MODAL BUAT TANDA TANGAN (KAPRODI) --}}
...
{{-- SCRIPT KANVAS KHUSUS JIKA TTD BELUM ADA --}}
@if(empty(Auth::user()->signature))
    <script>
