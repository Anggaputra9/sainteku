<template x-teleport="#modal-root">
    <div x-data="openRoomDeleteModal()"
        @open-delete-modal.window="handleOpenDelete($event)"
        x-show="openDelete"
        ...
        <div @click.away="openDelete = false"
            ...
            <div class="border-b border-gray-200 bg-white px-4 py-3 sm:px-6 sm:py-4 dark:bg-[#1e293b] dark:border-gray-700">
                <div class="flex min-w-0 items-center gap-2 sm:gap-3">
                    ...
                    <div class="min-w-0 leading-tight">
                        ...
                        <p class="truncate text-xs text-gray-500 dark:text-gray-400 sm:text-sm" x-text="roomTitle"></p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 space-y-5 bg-slate-50 dark:bg-[#0f172a]">
                <p class="text-sm text-gray-600 dark:text-gray-300">
                    Yakin ingin menghapus ruang ujian <span class="font-bold text-gray-900 dark:text-white" x-text="roomTitle"></span>?
                    Tindakan ini tidak dapat dibatalkan.
                </p>

                <div class="flex flex-row flex-nowrap items-center justify-end gap-2 pt-2">
                    <button type="button" @click="openDelete = false" :disabled="submitting"
                        class="inline-flex shrink-0 items-center justify-center rounded-xl bg-gray-200 px-3 py-2 text-xs font-bold text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 sm:px-5 sm:py-2.5 sm:text-sm transition disabled:opacity-60">
                        Batal
                    </button>
                    <button type="button" @click="submitDelete()" :disabled="submitting"
                        class="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-xl bg-red-600 px-3 py-2 text-xs font-bold text-white shadow-sm hover:bg-red-700 sm:gap-2 sm:px-6 sm:py-2.5 sm:text-sm transition disabled:opacity-60">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('openRoomDeleteModal', () => ({
            openDelete: false,
            deleteUrl: '',
            roomTitle: '',
            submitting: false,
            ...
            handleOpenDelete(event) {
                ...
                this.roomTitle = event.detail?.title || 'ruang ujian ini';
            ...
            async submitDelete() {
                ...
                try {
                    ...
                    window.dispatchEvent(new CustomEvent('rooms-data-changed', {
                        bubbles: true,
                        detail: { message: data.message || 'Ruang ujian dihapus.' },
                    }));
                } catch (e) {
                    window.dispatchEvent(new CustomEvent('rooms-data-changed', {
                        bubbles: true,
