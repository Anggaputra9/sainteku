...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    <div class="space-y-6">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {{-- CUKUP 1 TOMBOL (HIJAU, PAKAI MODAL) --}}
            <div x-data="{ openCreateDosen: false }">
                ...
                {{-- Include modal create dosen (warna hijau) --}}
                @include('manajemenachievement::dosen.modal-create', [
                'kategori' => $kategori,
                'tingkat' => $tingkat
                ])
            </div>
        ...
        <div class="flex flex-wrap items-center gap-2 mb-2">
            <a href="{{ route('dosen.repository.index') }}"
                class="px-4 py-2 text-sm font-semibold rounded-full transition {{ !request('status') ? 'bg-green-600 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700 dark:hover:bg-gray-700' }}">
                Semua Prestasi
            </a>
            <a href="{{ route('dosen.repository.index', ['status' => 'pending']) }}"
            ...
            <a href="{{ route('dosen.repository.index', ['status' => 'approved']) }}"
            ...
            <a href="{{ route('dosen.repository.index', ['status' => 'rejected']) }}"
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    {{-- Lihat Detail --}}
                                    <a href="{{ route('dosen.repository.show', $item->id) }}"
                                    ...
                                    {{-- Download File --}}
                                    @if($item->file_path)
                                    <a href="{{ route('dosen.repository.download', $item->id) }}"
                                        class="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-2 text-xs font-bold text-gray-700 hover:bg-gray-50 transition shadow-sm border border-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700"
                                        title="Download File">
                                    ...
                                    {{-- Edit (hanya pending) --}}
                                    @if($item->status == 'pending')
                                    <a href="{{ route('dosen.repository.edit', $item->id) }}"
                                    ...
                                    {{-- Hapus (hanya pending) --}}
                                    @if($item->status == 'pending')
                                    <form method="POST" action="{{ route('dosen.repository.destroy', $item->id) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center gap-3 text-gray-400 dark:text-gray-500">
                                    ...
                                    {{-- Di empty state TIDAK ADA TOMBOL, karena sudah ada di header --}}
                                </div>
                            </td>
                        </tr>
                        @endforelse
