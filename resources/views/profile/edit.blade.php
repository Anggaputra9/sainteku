...
@section('content')
    <div class="mx-auto" x-data="{ 
                    ...
                    sigCtx: null,
                    alert: { type: '', message: '' },
                    sigAlert: { type: '', message: '' },
                    flash(type, message) {
                        this.alert = { type, message };
                        setTimeout(() => { this.alert.message = ''; }, 4000);
                    },
                    sigFlash(type, message) {
                        this.sigAlert = { type, message };
                        setTimeout(() => { this.sigAlert.message = ''; }, 4000);
                    },
                    clearSigAlert() {
                        this.sigAlert = { type: '', message: '' };
                    },
                    ...
                    submitSignatureForm(form) {
                        if (this.sigMode === 'draw' && !this.sigHasDrawn) {
                            this.sigFlash('warning', 'Gambar tanda tangan di kanvas terlebih dahulu.');
                            return;
                        }
                        if (this.sigMode === 'upload' && !this.$refs.profileSignatureFile?.files?.length) {
                            this.sigFlash('warning', 'Pilih file gambar tanda tangan terlebih dahulu.');
                            return;
                        }
                        this.clearSigAlert();
            ...
            <div x-show="signatureOpen"
                ...
                <div @click.away="closeSignatureModal()"
                    ...
                    <form action="{{ route('profile.signature.store') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <template x-if="sigAlert.message">
                            <div class="mb-4 flex items-center gap-2.5 rounded-lg border-l-4 p-3 shadow-sm"
                                :class="{
                                    'border-amber-500 bg-amber-50 text-amber-800 dark:bg-amber-900/20 dark:text-amber-400': sigAlert.type === 'warning',
                                    'border-red-500 bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400': sigAlert.type === 'error',
                                    'border-green-500 bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400': sigAlert.type === 'success'
                                }">
                                <i class="fa-solid shrink-0"
                                    :class="{
                                        'fa-triangle-exclamation': sigAlert.type === 'warning',
                                        'fa-circle-xmark': sigAlert.type === 'error',
                                        'fa-check-circle': sigAlert.type === 'success'
                                    }"></i>
                                <span class="text-sm font-semibold" x-text="sigAlert.message"></span>
                            </div>
                        </template>

                        <div class="mb-4 flex gap-2 rounded-lg bg-gray-100 p-1 dark:bg-gray-900">
                            <button type="button" @click="clearSigAlert(); sigMode = 'draw'; setTimeout(() => initSigCanvas(), 50)"
                            ...
                            <button type="button" @click="clearSigAlert(); sigMode = 'upload'"
