            ...
            <div class="flex flex-col gap-2">
                <div>
                    ...
                    <ul class="flex flex-col gap-1.5">
                        ...
                        {{-- Tampilkan menu hanya jika ada child yang visible atau menu tanpa child --}}
                        @if(($hasChildren && $visibleChildren->count() > 0) || (!$hasChildren))
                        <li x-init="{{ $isActive && $hasChildren ? "if (open === null) open = $i;" : "" }}">

                            {{-- ================= PARENT WITH CHILD ================= --}}
                            @if($hasChildren && $visibleChildren->count() > 0)
                            ...
                            <a href="{{ $menuLink }}"
                                ...
                                <span class="flex items-center justify-center w-6 h-6 transition-transform duration-300 {{ $isActive ? 'scale-110' : 'group-hover:scale-110' }}">
                                    <i class="{{ $menu->menu_icon }} text-lg"></i>
                                </span>
