...
{{--
    PENTING: Semua nilai default array disiapkan di @php block.
    JANGAN taruh array literal [ ] atau operator ?? di dalam @json() —
    Blade compiler PHP akan gagal parse-nya.
--}}
@if($showMonev)
@php
    $chartLabels   = [];
    ...
    $chartApproved = [];
    $chartRevised  = [];
    foreach ($examByPeriod as $p) {
        $chartLabels[]   = $p->name . ' ' . $p->semester;
        $chartPending[]  = (int) $p->submitted_count;
        $chartApproved[] = (int) $p->approved_count;
        $chartRevised[]  = (int) $p->revised_count;
    }

    $safeTrendLabels   = isset($trendLabels)   ? $trendLabels   : ['Nov','Des','Jan','Feb','Mar','Apr'];
    $safeTrendPending  = isset($trendPending)  ? $trendPending  : [5, 8, 12, 9, 7, 24];
    $safeTrendApproved = isset($trendApproved) ? $trendApproved : [38, 42, 55, 48, 50, 62];
    $safeTrendRevised  = isset($trendRevised)  ? $trendRevised  : [8, 6, 11, 10, 7, 10];
@endphp
<script>
window._dashboardData = {
    examSubmitted : {{ $examSubmitted }},
    examApproved  : {{ $examApproved }},
    examRevised   : {{ $examRevised }},
    chartLabels   : @json($chartLabels),
    chartPending  : @json($chartPending),
    chartApproved : @json($chartApproved),
    chartRevised  : @json($chartRevised),
    trendLabels   : @json($safeTrendLabels),
    trendPending  : @json($safeTrendPending),
    trendApproved : @json($safeTrendApproved),
    trendRevised  : @json($safeTrendRevised)
};
</script>
@endif
        ...
        <div class="grid grid-cols-1 gap-6 p-6 md:grid-cols-2 xl:grid-cols-4">
            @if($showMonev)
                <div class="rounded-2xl border border-blue-200 bg-blue-50 p-5 dark:border-blue-800 dark:bg-blue-900/20">
                    ...
                    <p class="mt-3 text-4xl font-bold text-gray-900 dark:text-white">{{ number_format($examSubmitted) }}</p>
                ...
                <div class="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-800 dark:bg-emerald-900/20">
                    ...
                    <p class="mt-3 text-4xl font-bold text-gray-900 dark:text-white">{{ number_format($examApproved) }}</p>
                ...
                <div class="rounded-2xl border border-amber-200 bg-amber-50 p-5 dark:border-amber-800 dark:bg-amber-900/20">
                    ...
                    <p class="mt-3 text-4xl font-bold text-gray-900 dark:text-white">{{ number_format($examRevised) }}</p>
            ...
            @if($showInfrastructure)
                <div class="rounded-2xl border border-violet-200 bg-violet-50 p-5 dark:border-violet-800 dark:bg-violet-900/20">
                    ...
                    <p class="mt-3 text-4xl font-bold text-gray-900 dark:text-white">{{ number_format($infrastructurePending) }}</p>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-800/40">
                    ...
                    <p class="mt-3 text-4xl font-bold text-gray-900 dark:text-white">{{ number_format($infrastructureCompleted) }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">Sudah dikembalikan</p>
                </div>
            @endif

            @if(!$showMonev && !$showInfrastructure)
            ...
            <div class="p-6">
                ...
                <div class="relative w-full" style="height:240px;">
                    <canvas id="donutStatusChart" role="img" aria-label="Donut chart distribusi status tashih">
                        Pending: {{ $examSubmitted }}, Disetujui: {{ $examApproved }}, Direvisi: {{ $examRevised }}.
                    </canvas>
                </div>
            </div>
        </div>
        ...
        <div class="p-6">
            <table class="w-full text-left text-sm text-gray-600 dark:text-gray-400">
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                    @forelse($examByPeriod as $period)
                        <tr>
                            <td class="px-2 py-4 font-medium text-gray-900 dark:text-white">{{ $period->name }} &mdash; {{ $period->semester }}</td>
                            <td class="px-2 py-4 text-right font-semibold">{{ number_format($period->total_count) }}</td>
                            <td class="px-2 py-4 text-right text-blue-600 dark:text-blue-400">{{ number_format($period->submitted_count) }}</td>
                            <td class="px-2 py-4 text-right text-emerald-600 dark:text-emerald-400">{{ number_format($period->approved_count) }}</td>
                            <td class="px-2 py-4 text-right text-amber-600 dark:text-amber-400">{{ number_format($period->revised_count) }}</td>
                        </tr>
                    @empty
                        <tr>
