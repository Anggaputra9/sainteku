...
<div class="relative" x-data="{
    ...
    @php
        // 1. Tarik data notifikasi asli dari Database
    ...
    <button
        ...
        <span x-show="{{ $unreadCount > 0 ? 'true' : 'false' }}"
            class="absolute right-0 top-0.5 z-1 h-2 w-2 rounded-full bg-orange-400" style="display: none;">
            <span
    ...
    <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-100"
        ...
        <div class="flex items-center justify-between pb-3 mb-2 border-b border-gray-100/50 dark:border-gray-800/50">
            ...
            <div class="flex items-center gap-3">
                @if($unreadCount > 0)
                    <span class="px-2 py-0.5 text-[10px] font-bold text-white bg-orange-500 rounded-full">{{ $unreadCount }}
                        Baru</span>
                @endif
        ...
        <ul class="flex flex-col overflow-y-auto custom-scrollbar flex-1">
            @forelse ($notifications as $notification)
                <li>
                    {{-- 2. Logic Pindah Halaman / Buka Modal via AlpineJS --}}
                    <button type="button" @click="
                                closeDropdown(); 
                                if('{{ $notification->data['click_action'] ?? '' }}' === 'open_tashih_modal' && '{{ $notification->data['reference_id'] ?? '' }}') {
                                    if(window.location.pathname.includes('/tashih')) {
                                        document.querySelector('[x-data=tashihApp]').__x.$data.viewDetail('{{ $notification->data['reference_id'] }}', 'review');
                                    } else {
                                        window.location.href = '{{ $notification->data['target_url'] ?? '#' }}?open_modal={{ $notification->data['reference_id'] }}';
                                    }
                                } else {
                        ...
                        <span
                            ...
                            {{ $notification->data['userInitial'] ?? 'S' }}

                            {{-- Status Titik (Pake class bawaan lu bg-success-500/bg-error-500) --}}
                            <span
                                class="absolute bottom-0 right-0 z-10 h-2.5 w-full max-w-2.5 rounded-full border-[1.5px] border-white dark:border-gray-900 {{ ($notification->data['status'] ?? 'online') === 'online' ? 'bg-success-500' : 'bg-error-500' }}"></span>
                        </span>

                        <span class="block flex-1">
                            <span class="mb-1.5 block text-theme-sm text-gray-500 dark:text-gray-400">
                                <span class="font-medium text-gray-800 dark:text-white/90">
                                    {{ $notification->data['userName'] ?? 'Sistem' }}
                                </span>
                            ...
                            <span class="flex items-center gap-2 text-gray-500 text-theme-xs dark:text-gray-400">
                                <span>{{ $notification->data['type'] ?? 'Info' }}</span>
                                <span class="w-1 h-1 bg-gray-400 rounded-full"></span>
                                {{-- diffForHumans = otomatis nulis "5 min ago" --}}
